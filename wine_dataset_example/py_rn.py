...
def train_score_anfis():
    ...
    for i in h_n:
        ...
        for j in range(0, rep):
            ...
            ej = np.subtract(anfis_wine.Y_test[:, 0], np.transpose(anfis_wine.pred))
            total = len(Y_test[:, 0])
            acc = len(np.where(ej == 0)[0])
            print(i)
            acc_100 += acc * 100.0 / total
...
def compare_nets():
    ...
    plt.plot(
        ...
        anfis_wine.Y_test[:, 0],
        "b.",
        label="Classe real correspondente",
    )
    ...
    plt.plot(
        mlp.Y_test[:, 1], mlp.Y_test[:, 0], "b.", label="Classe real correspondente"
    )
    plt.xlabel("Index de entrada da rede")
    plt.ylabel("Classe da saída")
    plt.grid()
